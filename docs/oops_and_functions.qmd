...
#subset to only keep rows where column_name matches fish_name, otherwise it will consider milkfish for all three (returns three calculations for other species)!
  parameters <- fish_parms[fish_parms$common_name == fish_name, ] # keep columns, but subset for rows that match argument
  for(i in seq_along(parameters)) {
  for(j in seq_along(fish_lengths)) {
  # fish_weights_store[i] <- parameters$a_est*(fish_lengths[i]^parameters$b_est)
    W  <- fish_weight(fish_name, tot_length = fish_lengths[i])
   fish_weights_store[i] <- W
     break
...
fish_weight_2(fish_name = "Milkfish", fish_lengths)
